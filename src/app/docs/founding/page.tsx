...
import { getFoundingDocuments, strapiMedia, type FoundingDocument } from "@/lib/strapi";

export default function FoundingDocsPage() {
  const { t, locale } = useI18n();
  const [docs, setDocs] = useState<FoundingDocument[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    getFoundingDocuments(locale).then((data) => { setDocs(data); setLoading(false); }).catch(() => setLoading(false));
  }, [locale]);
        ...
        <div className="mx-auto max-w-6xl px-6 py-16">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="rounded-2xl border border-tam-grey/40 p-8">
                <p className="text-tam-black/50">{t("docs.founding.placeholder")}</p>
              </div>
            ) : (
              docs.map((doc) => (
